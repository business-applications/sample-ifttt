...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "sample-ifttt-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name sample-ifttt-service apps/sample-ifttt-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app sample-ifttt-service:1.0-SNAPSHOT
		call oc expose svc/sample-ifttt-service
	) else (
		echo "Launching the application locally..."
		cd sample-ifttt-service
		cd target
