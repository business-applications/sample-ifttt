...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "sample-ifttt-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name sample-ifttt-service apps/sample-ifttt-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app sample-ifttt-service:1.0-SNAPSHOT
    oc expose svc/sample-ifttt-service
else

	echo "Launching the application locally..."
	pattern="sample-ifttt-service"
	files=( $pattern )
	cd ${files[0]}
